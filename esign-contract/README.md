...
```
你：帮我写一份借款合同，甲方张三，乙方李四，金额5万，年利率8%，一年期
```
...
首次使用时 Skill 会自动引导配置，也可手动创建：

```bash
mkdir -p ~/.config/esign-contract
cat > ~/.config/esign-contract/.env << 'EOF'
...
> 前往 [e签宝开放平台](https://open.esign.cn) 获取应用凭证。沙箱环境将 `ESIGN_BASE_URL` 改为 `https://smlopenapi.esign.cn`。
...
- **发起签署**和**撤销签署**是不可逆操作，执行前会要求用户确认
- 当前版本支持双方合同（甲方、乙方）
- 默认适用中华人民共和国法律
- 凭证存储于 `~/.config/esign-contract/.env`，不会随项目提交
